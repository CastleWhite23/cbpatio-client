  ...
  return (
    <div>
        <Swiper 
            slidesPerView={1}
            autoplay={{
                delay: 3000, // tempo de intervalo em milissegundos (3 segundos)
            ...
            campeonatos.map((campeonato, index) => {
                return (<SwiperSlide key={index}>
                    {console.log(`${path}/${campeonato.foto}`)}
                        <CardCampeonato idCamp={campeonato.id_campeonato} bgImage={`${path}/${campeonato.foto.replace(/\\/g, '/')}`} title={campeonato.nome} height={"25rem"} width={"100%"}/>
                </SwiperSlide>)
            })
        }
